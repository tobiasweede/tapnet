...
def train(args, model, model_params, optimizer):
    """Train TapNet model"""
    ...
    for epoch in range(args.epochs):
        ...
        #     output[idx_val], torch.squeeze(labels[idx_val])
        # )
...
def validate(args, model, model_params):
    """Validate TapNet model"""
    features, labels, idx_train, idx_val, idx_test = model_params
    output, proto_dist = model(model_params)
    # loss_val = F.cross_entropy(
    #     output[idx_test], torch.squeeze(labels[idx_test])
    # )

    loss_val = focal_loss(output[idx_test], labels[idx_test])
    if args.use_metric:
        loss_val = loss_val - args.metric_param * proto_dist

    # acc_val = accuracy(output[idx_test], labels[idx_test])
    fbeta_val = fbeta(output[idx_test], labels[idx_test])
    cm_val = confusion_matrix(output[idx_test], labels[idx_test])
    print(
        args.dataset,
...
def main():
    ...
    t_total = time.time()
    train(args, model, model_params, optimizer)
    print("Optimization Finished!")
    print("Total time elapsed: {:.4f}s".format(time.time() - t_total))
    validate(args, model, model_params)
